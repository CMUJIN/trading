...
------------------------------------------------------------------------------
Stable GitHub version (Unified with chipzones hybrid v3.8.2)
Output:
    docs/<SYMBOL>/<SYMBOL>_trend_v6_YYYYMMDD_HH.png
Automatically clears old trend_v6 files in docs/<SYMBOL>/.
"""
...
def detect_trends_v6(prices, oi, alpha, min_move, min_slope, min_oi_delta,
                     min_oi_up, min_bars, dir_ratio):
    n = len(prices)
    segs = []
    ...
    for s in range(0, n - min_bars):
        for e in range(s + min_bars, n):
            ...
            if (move1_dir >= min_move and slope1 >= min_slope
                and oi_delta1 >= min_oi_delta and oi_up1 >= min_oi_up):
    ...
    filtered = []
    for seg in segs:
        if not any(o['s'] <= seg['s'] <= o['e'] or o['s'] <= seg['e'] <= o['e']
                   for o in filtered):
            filtered.append(seg)
...
# ---------- Delete old trend_v6 files ----------
def delete_old_trend_files(symbol):
    save_dir = f"docs/{symbol}"
    if not os.path.exists(save_dir):
        return

    for f in os.listdir(save_dir):
        if "trend_v6" in f and f.endswith(".png"):
            os.remove(os.path.join(save_dir, f))
            print(f"[INFO] 删除旧 trend 文件：{save_dir}/{f}")


# ---------- CLI Entrypoint ----------
def main():
    parser = argparse.ArgumentParser(description="Trend Detection v6 (Unified output folder)")
    ...
    oi_col = next((c for c in df.columns if any(k in c for k in
                   ["open_interest", "oi", "position", "hold", "持仓"])), None)
    if oi_col is None:
    ...
    # --- Trend detection ---
    segs = detect_trends_v6(
        prices, oi, args.alpha, move_thresh, slope_thresh,
        args.delta_oi, args.oi_up, args.min_bars, args.dir_ratio
    )
    ...
    for seg in segs:
        ...
        axes[0].plot(df["bar"].iloc[seg["s"]:seg["e"]+1],
                     prices[seg["s"]:seg["e"]+1],
                     color=c, lw=2.5)

    axes[0].set_title(
        f"Trend v6 (α={args.alpha}, slope≥{slope_thresh*100:.3f}%/bar, "
        f"move≥{move_thresh*100:.2f}%, vol={volatility_base*100:.1f}%)",
        fontsize=13
    )
    ...
    axes[1].plot(df["bar"], oi, color='blue', lw=1.6,
                 label=f"OI (EMA{args.oi_ema_span})")
    for seg in segs:
        c = 'green' if seg['dir'] == 1 else 'red'
        axes[1].axvspan(df["bar"].iloc[seg["s"]],
                        df["bar"].iloc[seg["e"]],
                        color=c, alpha=0.15)

    axes[1].set_xlabel("Trading Bars")
    ...
    # === 统一输出目录 docs/<symbol>/ ===
    symbol = os.path.basename(args.file).split("_")[0].upper()
    save_dir = f"docs/{symbol}"
    os.makedirs(save_dir, exist_ok=True)

    # === 删除旧 trend_v6 文件 ===
    delete_old_trend_files(symbol)

    # === 保存新图 ===
    out = f"{save_dir}/{symbol}_trend_v6_{ts}.png"
